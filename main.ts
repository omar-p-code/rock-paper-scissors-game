type ElementName = 'rock' | 'paper' | 'scissors' | 'lizard' | 'spock';
...
if (localStorage.getItem('score')) {
   let score: any = localStorage.getItem("score") ? JSON.parse(localStorage.getItem("score")!) : 0;
   const scoreElement = document.querySelector('header .score span:last-child');
   if (scoreElement) {
...
let rulesBtn: HTMLDivElement = document.querySelector('.rules .rules-btn') as HTMLDivElement;
let rules: HTMLDivElement = document.querySelector('.rules-modal') as HTMLDivElement;
let rulesClose: HTMLDivElement = document.querySelector('.rules-modal .close-btn svg') as HTMLDivElement;
...
let gameElements: HTMLDivElement[] = Array.from(document.querySelectorAll('.game .polygon .ele')) as HTMLDivElement[];

gameElements.forEach((ele: HTMLDivElement): void => {
   ele.addEventListener('click', (event: MouseEvent): void => {
      let selectedElement: ElementName = (event.currentTarget as HTMLDivElement).classList[0] as ElementName;
      let computerElement: ElementName = getRandomElement();
      document.querySelector('.game')?.classList.add('picked');
      displayPlayer(selectedElement);
      
      setTimeout(() => {
         displayComputer(computerElement);
      }, 2000);
      
      setTimeout(() => {
         let result: string = determineWinner(selectedElement, computerElement);
         displayResult(result);
         document.querySelector('.game')?.classList.add('picked-result');
      }, 4000);
   });
})

// console.log(getRandomElement()); // For testing purposes


function getRandomElement(): ElementName {
   let keys: ElementName[] = Object.keys(elements) as ElementName[];
   let randomIndex: number = Math.floor(Math.random() * keys.length);
   return keys[randomIndex];
}

/**
 * @param player The Player Chosen Element
 * @param computer The Computer Chosen Element
 * @returns The Result Lose | Win | Draw
 * @description It Edit The Score And Save It Into The localStorage And Play The Efict Sound For Each One 
 */
function determineWinner(player: ElementName, computer: ElementName): string {
      if (elements[player].beats.includes(computer)) {
         score++
         localStorage.setItem('score', JSON.stringify(score));
         new Audio('dist/win.mp3').play();
         return 'You Win';
...
// console.log(determineWinner('rock', getRandomElement())); // For testing purposes

/**
 * @param player The Player Chosen Element 
 * @description view The Player Chosen Element
 */
function displayPlayer(player: ElementName): void {
   let playerEle: HTMLDivElement = document.querySelector('.game .action .player .icon') as HTMLDivElement;
   playerEle.innerHTML = elements[player].icon;
   if (playerEle.parentElement) {
      playerEle.parentElement.classList.add(player);
   }
}

/**
 * @param computer The Comuter Chosen Element 
 * @description View The Computer Chosen Element
 */
function displayComputer(computer: ElementName): void {
   let computerEle: HTMLDivElement = document.querySelector('.game .action .computer .icon') as HTMLDivElement;
   computerEle.innerHTML = elements[computer].icon;
   if (computerEle.parentElement) { 
      computerEle.parentElement.classList.add(computer);
   }
}
...
function displayResult(result: string): void {
   let resultText: HTMLDivElement = document.querySelector('.game .result-text') as HTMLDivElement;
   resultText.innerText = result;
   document.querySelector('header .score span:last-child')!.innerHTML = JSON.stringify(score)
}

// displayResult('rock', getRandomElement(), determineWinner('rock', getRandomElement())) // For testing purposes
// For testing purposes, you can uncomment the line below to see the initial game state


document.querySelector('.game .result .result-btn')?.addEventListener('click', playAgain);

/**
 * @description Reset All Values To Be Ready To Play Again
 */
function playAgain() {
   document.querySelector('.game')?.classList.remove('picked', 'picked-result');
   document.querySelector('.game .action .player')?.classList.remove(...Object.keys(elements));
   document.querySelector('.game .action .computer')?.classList.remove(...Object.keys(elements));
   document.querySelector('.game .action .player .icon')!.innerHTML = '';
   document.querySelector('.game .action .computer .icon')!.innerHTML = '';
   document.querySelector('.game .result-text')!.innerHTML = '';
}